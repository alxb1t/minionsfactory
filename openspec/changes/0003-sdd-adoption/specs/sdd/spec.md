...
The new capability this change adds: an **enforced** binding of behavioral scenarios to proving tests, checked at
**gate → reviewer → release**, plus the change lifecycle (`changes/<id>/` structure, delta fold, commit
traceability) that carries it. Format is OpenSpec verbatim; each scenario carries an explicit stable `Key:` and
declares its `Layers:` (v0.3 enforces `unit`; `e2e` is reserved for the v0.7 tester — recorded, not enforced).
