...
```python
provider = ClaudeCodeProvider()  # or FakeProvider(preset) in tests
...
Pure. Assembles the `claude -p … --output-format json --permission-mode <mode>` argv, appending
`--allowedTools` / `--disallowedTools` only when the profile carries them.
