...
The deterministic driver — **no LLM** — that advances the plan phase by phase or halts. The verdict is a pure
function; the loop is thin orchestration over the seams.
...
- **Advance requires both** a new commit (`head` changed) **and** a moved `current_phase` — a green gate alone
  is not enough.
...
```python
def decide(before: PlanState, after: PlanState, gate_result: GateResult, coder_halted: bool) -> Decision
```

The pure verdict for one phase. Precedence: **halt-report → red-gate → non-advance → advance**, where advance
requires **both** a new commit and a moved `current_phase`.

- **Params** — [`before`/`after`](state.md#planstate): plan state around the phase · [`gate_result`](gate.md#gateresult): the gate verdict · `coder_halted`: did the coder write a HALT report.
