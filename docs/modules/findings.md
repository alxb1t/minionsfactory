...
public_api: [read_findings_state, FindingsState]
depends_on: [state]
---
...
converged = all(s is not None and s.verdict == "clean" for s in states)
```
...
- **Why Pydantic, not a dataclass** — unlike its sibling [`PlanState`](state.md#planstate), this boundary read
  ...
  at every boundary.
- **Consumed by** — [`converge`](converge.md#converge) (clean iff `verdict == "clean"` for all three);
