...
at a target repo, it drives an **in-repo change** to completion: a fresh, per-phase **coder** builds the change's
`tasks.md` phase by phase (the orchestrator runs the quality gate itself and commits on green); at the end of the
change it fans out **review ‖ security ‖ simplify** as fresh read-only instances, runs a **converge loop** over their
blocking findings, then a **release** stage verifies the release gate and prepares the release locally — folding the
change's spec delta into the living `openspec/specs/` and halting for the human to merge + push (the boundary never
crosses). Every role is a fresh, single-role Claude Code instance behind a **provider seam**; the driver is
deterministic control flow that **advances or halts** on machine-checkable disk state.
...
6. **The framework's own green gate is the definition of done** — the six-step command list declared at
   `.minions/minions.toml`, run in order, dogfooded on itself. That array is the gate; prose that enumerates
   it is a mirror, never the source.
...
graph TD
    ...
    state["state — change-state reader"]
